...
export default function BookingWidget({ venueId }: BookingWidgetProps) {
  const venue =
    mockVenues.find((v) => v.id.toString() === venueId) ?? mockVenues[0];
        ...
        <div>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {slotsForDate.map((slot, idx) => {
              ...
              const baseClasses =
                "w-full px-4 py-3 rounded-lg border-2 transition-all text-left font-medium";
              let stateClasses = "";
              if (!isAvailable) {
                stateClasses =
                  "border-border bg-muted opacity-50 cursor-not-allowed text-muted-foreground";
              } else if (isSelected) {
              ...
              // Format time range (e.g., "06:00" -> "06:00 - 07:00")
              const formatTimeRange = (time: string) => {
                const [hours, minutes] = time.split(":").map(Number);
                const endHour = (hours + 1) % 24;
                const formattedEnd = `${endHour
                  .toString()
                  .padStart(2, "0")}:${minutes.toString().padStart(2, "0")}`;
                return `${time} - ${formattedEnd}`;
              };
            ...
            <span className="font-semibold text-foreground">
              {(() => {
                ...
                const formattedEnd = `${endHour
                  .toString()
                  .padStart(2, "0")}:${minutes.toString().padStart(2, "0")}`;
                return `${selectedSlot} - ${formattedEnd}`;
              })()}
            </span>
          </div>
